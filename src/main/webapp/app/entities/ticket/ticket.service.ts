...
export class TicketService {
    ...
    protected convertDateFromClient(ticket: ITicket): ITicket {
        const copy: ITicket = Object.assign({}, ticket, {
            orderDate: ticket.orderDate != null && ticket.orderDate.isValid() ? ticket.orderDate.toJSON() : null
        });
        return copy;
    }
    ...
    protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
        if (res.body) {
            res.body.forEach((ticket: ITicket) => {
                ticket.orderDate = ticket.orderDate != null ? moment(ticket.orderDate) : null;
            });
        }
        return res;
    }
